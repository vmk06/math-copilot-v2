...
function parseAIResponse(text: string) {
  const pull = (name: string) => {
    // IMPORTANT: double backslashes in \\s\\S
...
export default function Page() {
  ...
  const [showSolution, setShowSolution] = useState(false); // gate the solution
  ...
  async function onSubmit(e?: FormEvent) {
    ...
    try {
      ...
      // Fallback: if the model forgot tags, show the whole text as the solution
      if (parsed.hints.length === 0 && !parsed.solution) {
        setHints([]);
        setSolution(raw.trim());
        setShowSolution(true); // auto-reveal so the user sees something
      } else {
  ...
  return (
    <main className="min-h-screen bg-slate-900 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        {hints.length > 0 && (
          <section className="mt-8">
            ...
            <div className="mt-2 flex flex-wrap gap-3">
              ...
              {!showSolution && (hints.length > 0 || solution) && (
                <button
                  type="button"
        ...
        {/* If no hints but a solution exists (fallback), still show Reveal button */}
        ...
        <section className="mx-auto mt-10 max-w-2xl text-slate-200">
          <div className="rounded-2xl border border-slate-700/60 bg-slate-800/60 p-5">
            ...
            <p className="mt-2 text-slate-300">
              {PRODUCT} helps beginners <span className="font-semibold">learn by thinking</span>. Instead of dumping full
              solutions, it uses <span className="font-semibold">staged hints → strategy → full solution</span> so you
              stay in control of the reasoning.
            </p>
        ...
        <section id="teacher" className="mx-auto mt-8 max-w-2xl text-slate-200">
          <div className="rounded-2xl border border-slate-700/60 bg-slate-800/40 p-5">
            ...
            <p className="mt-2 text-slate-300">
              We’re building a teacher mode with worksheet generation, hint levels, printable keys, and class tracking.
              If you’d like early access or to share feedback, reach out!
            </p>
